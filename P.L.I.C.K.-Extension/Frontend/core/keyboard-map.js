...
function convertText(text) {
  ...
  return chars.map(ch => {
    // Thai character → แปลง Thai→English
    if (/[\u0E00-\u0E7F]/.test(ch)) {
      lastLang = "th";
      return thaiToEng[ch] || ch;
    }
    // English letter → แปลง English→Thai
    if (/[a-zA-Z]/.test(ch)) {
      lastLang = "en";
      return engToThai[ch] || ch;
    }
    // สัญลักษณ์/ตัวเลข → ใช้บริบทจากตัวอักษรก่อนหน้า
    const ctx = lastLang || (overallThai ? "th" : "en");
    if (ctx === "th") return thaiToEng[ch] || ch;
    return engToThai[ch] || ch;
  }).join("");
}
