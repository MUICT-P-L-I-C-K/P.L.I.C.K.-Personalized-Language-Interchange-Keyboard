...
async def detect_lang_mistake(req: DictCheckRequest):
    ...
    # ── Step 3: หาคำแนะนำ (หาเฉพาะเมื่อจำเป็นเพื่อลดเวลา) ──
    # ถ้าคำที่เราพิมพ์มันผิด (ไม่เจอในดิก) และคำที่แปลงแล้วก็ไม่เจอในดิกด้วย ค่อยหาข้อเสนอแนะตัวสะกด
    if not original_exists:
        original_suggestions = await loop.run_in_executor(
            executor, get_spell_suggestions_sync, word, detected_lang
        )

        # เราจะหาคำแนะนำฝั่งตรงข้าม ก็ต่อเมื่อมันไม่มีความหมายทั้งคู่ 
        # (ถ้าฝั่งตรงข้ามมีความหมายอยู่แล้ว converted_exists=True ไม่จำเป็นต้องใช้เวลาหาข้อเสนอแนะ)
