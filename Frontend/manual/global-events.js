...
document.addEventListener("mouseup", async () => {
  ...
  setTimeout(async () => {
    ...
    if (correctionEnabled || autoDetectEnabled) {
      const result = await detectLanguageMistake(sel.text);
      if (!result) return;

      const isOriginalCorrect = result.original && result.original.exists;
      const hasSpell = correctionEnabled && !isOriginalCorrect && result.original.suggestions?.length > 0;
      const hasLang = autoDetectEnabled && !isOriginalCorrect && result.converted && (result.converted.exists || result.converted.suggestions?.length > 0);

      if (hasSpell || hasLang) {
        showCombinedBubble(result, sel, null, sel.text);
      }
    }
  }, 0);
});
...
document.addEventListener("keyup", (e) => {
  ...
  _selectionKeyTimer = setTimeout(async () => {
