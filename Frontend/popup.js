document.addEventListener("DOMContentLoaded", () => {
  ...
  function setLanguage(lang) {
    ...
    if(closeServerBtn) closeServerBtn.innerText = t.closeServer;
  }
  ...
  if (closeServerBtn) {
    closeServerBtn.addEventListener("click", () => {
      chrome.runtime.sendMessage({ action: "shutdown" }, (res) => {
        const lang = langToggle && langToggle.checked ? "th" : "en";
        alert(lang === "th" ? "ปิดเซิร์ฟเวอร์เรียบร้อยแล้ว" : "Server has been closed.");
  ...
  // Set an interval to run every 3 seconds to keep updating
